...
TEST(JitLoggingLevelsTest, CheckSetLoggingLevel)  {
  ...
  ASSERT_TRUE(::torch::jit::is_enabled("file_to_test.cpp", JitLoggingLevels::GRAPH_DUMP));
}
...
TEST(JitLoggingLevelsTest, CheckSetMultipleLogLevels) {
  ...
  ASSERT_TRUE(::torch::jit::is_enabled("f2.cpp", JitLoggingLevels::GRAPH_UPDATE));
  ASSERT_TRUE(::torch::jit::is_enabled("f3.cpp", JitLoggingLevels::GRAPH_DEBUG));
}
...
TEST(JitLoggingLevelsTest, CheckLoggingLevelAfterUnset) {
  ...
  ASSERT_FALSE(::torch::jit::is_enabled("f1.cpp", JitLoggingLevels::GRAPH_DUMP));
}

TEST(JitLoggingLevelsTest, CheckAfterChangingLevel) {
  ::torch::jit::set_jit_logging_levels("f1");
  ::torch::jit::set_jit_logging_levels(">f1");
  ASSERT_TRUE(::torch::jit::is_enabled("f1.cpp", JitLoggingLevels::GRAPH_UPDATE));
}
